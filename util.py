...
def load_data(path, valid_split=0.1):
  ...
  selected_features = np.sort(np.argsort(np.count_nonzero(raw_data, axis=0))[15:])
...
def transform_feat(raw_feat, selected_features):
  ...
  qualified_data_idxs = [j for j in range(raw_feat.shape[0]) if np.count_nonzero(feat[j,:]) >= 72]
